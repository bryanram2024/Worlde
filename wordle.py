...
def generate_rand_word(level):
    # By randomizing, we can prevent ChatGPT reusing the same word each time the program runs.
    random_number = random.randint(1, 100)
    return open_ai(f"Give me a common word with a length of {level} that can be easily guessed for a Wordle game. Add this number for randomness: {random_number}. Only return just the lowercased word and nothing else.")
...
def get_user_guess(level, r_word, attempt, hint):
    while True:
        ...
        # if the user's attempt is correctly formatted, return and continue on with the program.
        if len(guess) == level and not any(char.isdigit() for char in guess):
            return guess
        
        # If user types in 'y', return a hint generated by ChatGPT
        elif guess == "y":
            if hint == level - 2:
        ...
        # Else, return to the user what to fix in their guess in order to continue the program.
...
def game(level, r_word):
    ...
    # Lists to hold all of user's guesses and its letter color correspondence.
    guesses = []
    guesses_colors = []
    ...
    while attempt < level:
        ...
        # Else, iterate and compare user's guess word to random word.
            ...
            # Append the guess and it's color in a 2D array to iterate throughout the program.
            guesses.append(user_guess)
            guesses_colors.append(color)
            

            # Print out all attempts with letters and their corresponding colors.
        ...
        # Finally, add to attempt to limit the number of attempts.
        attempt += 1
    
    # If number of attempts reaches the limit, return False and user loses the game.
    return False
